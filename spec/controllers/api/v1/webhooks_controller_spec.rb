...
RSpec.describe API::V1::WebhooksController, type: :request do
  ...
  describe 'POST #check_payment votes' do
    before do
      ...
      post '/api/v1/check_payment', params: params.to_json, headers: headers
    end
    ...
    context 'when transaction was found' do
      context 'when vote.value were enrolled' do
        let!(:transaction) { create :purchase_transaction, entry: entry, product_type: :vote, product: product_votes, intent_id: params[:data][:object][:id] }
      ...
      context 'when votes where bought' do
        let!(:transaction) { create :purchase_transaction, entry: entry, status: :done, product: product_votes, intent_id: params[:data][:object][:id] }
  ...
  describe 'post #check_payment spins' do
    before do
      allow(ENV).to receive(:fetch).with('STRIPE_ENDPOINT_SECRET', any_args).and_return('stripe_endpoint_secret')
      stripe_signature = generate_stripe_signature(params.to_json)
      headers = { 'Stripe-Signature' => stripe_signature }
    ...
    context 'when transaction was found' do
      context 'when vote.value were enrolled' do
        let!(:transaction) { create :purchase_transaction, product_type: :spin, product: product_spinner, intent_id: params[:data][:object][:id] }
      ...
      context 'when spins where bought' do
        let!(:transaction) { create :purchase_transaction, entry: entry, product: product_spinner, intent_id: params[:data][:object][:id] }
